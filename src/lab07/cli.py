...
class CLI:
    ...
    def _add_weapon(self) -> None:
        ...
        CLI отвечает только за ввод данных от пользователя.
        Создание объектов оружия делегировано в app (слой бизнес-логики).
        """
        print("\n--- Добавление оружия ---")
        print("Тип оружия:")
        ...
        try:
            ...
            # Создание объекта делегировано в app — CLI не инициализирует модели напрямую
            if choice == 1:
                weapon = self._app.create_weapon(damage, rare, hardness, effects, tip, imia, kolichestvo)
            elif choice == 2:
                caliber = float(input("Калибр (мм): "))
                magazine_capacity = int(input("Ёмкость магазина: "))
                weapon = self._app.create_firearms(damage, rare, hardness, effects, tip, imia,
                                                   kolichestvo, caliber, magazine_capacity)
            else:
                blade_length = float(input("Длина клинка (см): "))
                material = input("Материал: ").strip()
                weapon = self._app.create_edged(damage, rare, hardness, effects, tip, imia,
    ...
    def _filter_menu(self) -> None:
        ...
        if choice in filters:
            ...
            try:
                ...
                max_input = input("Максимальный урон (Enter для без ограничений): ").strip()
            ...
            rare = input("Редкость (Обычное/Редкое/Легендарное): ").strip()
